...
export default function EcosystemPage() {
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="animate-in flex gap-4 text-xs" style={{ animationDelay: `${0.05 * ai++}s` }}>
          ...
          {CONNECTIONS.map((conn, i) => {
            ...
            return (
              <line
                ...
                onClick={() => !conn.active && setSelected(conn)}
              />
            )
          })}
          {/* Nodes */}
      ...
      {selected && (
        <div className="animate-in rounded-lg border border-[var(--rose)]/30 bg-[var(--rose)]/5 p-5" style={{ animationDelay: '0s' }}>
          <div className="flex items-center justify-between mb-3">
            <span className="label text-[10px] tracking-widest text-[var(--rose)]">MISSING CONNECTION</span>
            <button onClick={() => setSelected(null)} className="text-xs text-[var(--text-dim)] hover:text-[var(--text)]">
              Close
            </button>
          </div>
          ...
          {selected.recommendation && (
            <p className="mt-3 text-sm text-[var(--text)] leading-relaxed">{selected.recommendation}</p>
          )}
        </div>
      )}
